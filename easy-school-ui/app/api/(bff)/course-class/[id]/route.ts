...
const clientApi = externalApiClient.resource('/course-classes');
...
export async function GET(req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const data = await clientApi.get<CourseClassModel>(id, {headers: await bearerHeaders() , cache: 'no-store' });
