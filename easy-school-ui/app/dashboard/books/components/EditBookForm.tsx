...
export default function EditBookForm({ book, onSave }: Props) {
  const router = useRouter();

  const [id, setId] = useState<string | undefined>(book?.id);
  const [name, setName] = useState(book?.name ?? '');
  const [status, setStatus] = useState<boolean>(Boolean(book?.status));
  const [chapters, setChapters] = useState<ChapterModel[]>(
    book?.chapters?.length ? book.chapters : [{ name: '' }]
  );
  ...
  useEffect(() => {
    setId(book?.id);
    setName(book?.name ?? '');
    setStatus(Boolean(book?.status));
    setChapters(book?.chapters?.length ? book.chapters : [{ name: '' }]);
    setChapterPage(1);
  }, [book]);
    ...
    <form onSubmit={handleSubmit} className="w-full">
      ...
      <div className="px-4 pt-2 md:px-0 md:pt-4">
        ...
        <p className="text-xs md:text-sm text-gray-500">
          Atualize o nome, o status e os capítulos do livro.
        </p>
      </div>
            ...
            <div>
              ...
              <div className="mt-2 space-y-2">
                {visibleChapters.map(({ chapter, index }) => (
                  <div key={chapter.id ?? `new-${index}`} className="flex items-center gap-2">
